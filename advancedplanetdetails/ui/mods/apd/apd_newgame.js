...
var oldAllPlanets = '<div class="all-planets">\
...
<img style="position:absolute;bottom:0px;right:0px;" src="../live_game/img/planet_list_panel/icon_engine_status_active.png" data-bind="visible: $root.apd.dataString($index(), \'thrusters\') > 0" width="10" />\
<div class="apd_details" style="position:absolute;border-image:url(coui://ui/main/shared/img/panel/img_menu_panel_secondary.png) 14 22 22 22;border-width:14px 22px 22px 22px;box-shadow: inset 1000px 1000px 1000px 1000px #1a1a1a;z-index:1;" data-bind="visible: $root.apd.currentPlanetIndex() === $index() && $root.apd.bShowPlanetDetails">\
...
newAllPlanets = '<div class="all-planets">\
<!-- ko foreach: biomes -->\
<div class="each-planet" style="position:relative;display:inline-block;" data-bind="event: { mouseover: $root.apd.setCurrentPlanet.bind($data, $index()), mouseout: $root.apd.bShowPlanetDetails(false) }">\
<img style="height: 20px; width: 20px;" data-bind="attr: { src: \'coui://ui/main/shared/img/planets/small/\' + $data + \'.png\' }" />\
<img style="position:absolute;bottom:0px;right:0px;" src="../live_game/img/planet_list_panel/icon_engine_status_active.png" data-bind="visible: $root.apd.dataString($index(), \'thrusters\') > 0" width="10" />\
...
oldPlanetTable = '<div class="apd_table" style="position:absolute;top:90px;left:355px;max-width:808px;display:none;padding: 10px 0 0 10px;border-image: url(\'img/panel_secondary.png\') 33 46 46 33 fill;border-width: 8px 32px 36px 12px;z-index:15;" data-bind="visible: apd.bShowDetailPanel">\
...
<div class="apd_table_planets_cont" style="padding:5px;white-space:nowrap;overflow:auto;-webkit-box-shadow:inset 0 0 10px #000;" data-bind="foreach: system().planets">\
...
<span style="position:absolute;bottom:0px;right:4px;font-size:75%;" data-bind="text: required_thrust_to_move, visible: required_thrust_to_move > 0"></span>\
 ...
 <div style="text-transform:uppercase;" data-bind="text: $root.apd.planetNameString(name)"></div>\
<div style="margin-bottom:1.0em;">\
<span class="input_label">Size:</span><span style="text-transform:uppercase;" data-bind="text: $root.apd.sizeString(planet.radius)"></span><br>\
<span class="input_label">Surf. Area:</span><span style="text-transform:uppercase;" data-bind="text: $root.apd.areaString(planet.radius)"></span><span>km&sup2;</span><br>\
<span class="input_label">Base Radius:</span><span style="text-transform:uppercase;" data-bind="text: planet.radius"></span><span>m</span><br>\
<span class="input_label">Random Seed:</span><span style="text-transform:uppercase;" data-bind="text: planet.seed"></span><br>\
<span class="input_label">Height Range:</span><span style="text-transform:uppercase;" data-bind="text: planet.heightRange"></span><br>\
<span class="input_label">Water Height:</span><span style="text-transform:uppercase;" data-bind="text: planet.waterHeight"></span><br>\
<span class="input_label">Temperature:</span><span style="text-transform:uppercase;" data-bind="text: planet.temperature"></span><br>\
<span class="input_label">Metal Density:</span><span style="text-transform:uppercase;" data-bind="text: planet.metalDensity"></span><span>%</span><br>\
...
showDetailsButton = '<div class="btn_std_gray btn_new_system" style="float: right; margin: 0px 2px 0px;" data-bind="click: apd.toggleDetailPanel"><div class="btn_std_label">Details</div></div>';
//

if ("processedSelectedSystem" in model) {
    $(".section_content_table").first().prepend(newPlanetTable);
    $("#system div.dropdown-system-picker .all-planets").replaceWith(newAllPlanets);
    $("#system .system-controls").before(showDetailsButton);
} else {
    $(".section_content_table").first().prepend(oldPlanetTable);
    $("#system div.all-planets").replaceWith(oldAllPlanets);
    $("#system .system-controls").before(showDetailsButton);
};
